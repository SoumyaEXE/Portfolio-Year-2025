...
const containerVariants = {
  hidden: { opacity: 0, scale: 0.75, y: 20, filter: 'blur(4px)' },
  visible: {
    opacity: 1,
    scale: 1,
    y: 0,
...
const itemVariants = {
  hidden: { opacity: 0, scale: 0.4, y: 15, rotate: -5 },
  visible: {
    opacity: 1,
    scale: 1,
    y: 0,
  ...
  exit: {
    ...
    y: 8,
    rotate: 5,
    transition: { duration: 0.15, ease: EASING.easeInOut }
  }
};

const shadowVariants = {
  initial: { boxShadow: '0 4px 15px rgba(0, 0, 0, 0.15)', backdropFilter: 'blur(8px)' },
  animate: { boxShadow: '0 12px 32px rgba(0, 0, 0, 0.25)', backdropFilter: 'blur(12px)' }
};
          ...
          >
            {REACTIONS.map((reaction) => (
              <motion.button
                ...
                onKeyDown={(e) => e.key === 'Enter' && onSelectReaction(reaction.emoji)}
              ...
              >
                ...
                />
                <motion.span style={{ display: 'inline-block' }} whileHover={animationConfigs.emojiHover}>
                  {reaction.emoji}
                </motion.span>
              </motion.button>
            ))}
